...
### items is a list of elements [w, v, d] (weights, values, and density)
	...
	k = 1 # We assume that all the knapsacks are normalized: the weights and the capacity all are divided by the capacity (so the capacity turns 1) 
	# and the values are all divided by the max value (so all of the values are between in the interval [0 and 1]).
...
class KnapsackInstances:
  # Generates instances of the Knapsack problem
	...
	def generateKnapsacks(self):
		...
		items_weights = list(itertools.combinations_with_replacement(base_weights_list,self.n_items)) # combinations_with_replacement allows us to put aside the same all the replica lists that only differ in the ordering
		...
		items_values = list(itertools.combinations_with_replacement(base_values_list,self.n_items)) # combinations_with_replacement allows us to put aside the same all the replica lists that only differ in the ordering
