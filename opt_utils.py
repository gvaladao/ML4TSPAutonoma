...
def useGurobi(items):
    ...
    k = 1
    # We assume that all the knapsacks are normalized: the weights and the capacity all are divided by the capacity (
    # so the capacity turns 1) and the values are all divided by the max value (so all the values are between in
    # the interval [0 and 1]).
    assert all([0 < w <= 1 for w in weights]), "weights should be between 0 and 1"
    assert all([0 < v <= 1 for v in values]), "values should be between 0 and 1"
    m = gp.Model()
    ...
    m.setObjective(quicksum(x[i] * values[i] for i in range(n)), GRB.MAXIMIZE)
    m.addConstr(quicksum(x[i] * weights[i] for i in range(n)) <= k)
    m.optimize()
    # how about multiple solutions?
...
class KnapsackInstances:
    ...
    def __init__(self, n_items, k, max_weight, max_value, numbasevalues):
    ...
    def generateKnapsacks(self):
        ...
        items_weights = list(itertools.combinations_with_replacement(base_weights_list,
                                                                     self.n_items))  # combinations_with_replacement allows us to put aside the same all the replica lists that only differ in the ordering
        ...
        items_values = list(itertools.combinations_with_replacement(base_values_list,
                                                                    self.n_items))  # combinations_with_replacement allows us to put aside the same all the replica lists that only differ in the ordering
